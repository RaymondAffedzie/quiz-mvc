...
class Session
{
	...
	/** new and improved put data into session **/
	public function set(mixed $keyOrArray, mixed $value = ''): int
	{
		$this->start_session();
		...
		if (is_array($keyOrArray)) {
			...
			foreach ($keyOrArray as $key => $val) {
				...
				if (is_array($val) && isset($_SESSION[$this->mainkey][$key])) {
				
					$_SESSION[$this->mainkey][$key] = array_merge($_SESSION[$this->mainkey][$key], $val);
				} else {
